-- Limpiar todo en orden con CASCADE
DROP TRIGGER IF EXISTS update_conversation_on_new_message ON public.maya_messages CASCADE;
DROP TRIGGER IF EXISTS update_maya_conversations_timestamp ON public.maya_conversations CASCADE;
DROP FUNCTION IF EXISTS public.update_maya_conversation_timestamp() CASCADE;
DROP FUNCTION IF EXISTS public.update_updated_at_timestamp() CASCADE;
DROP FUNCTION IF EXISTS public.user_owns_maya_conversation(UUID) CASCADE;
...
DROP TABLE IF EXISTS public.maya_messages CASCADE;
DROP TABLE IF EXISTS public.maya_conversations CASCADE;

-- Crear tabla maya_conversations
CREATE TABLE public.maya_conversations (
...
CREATE TABLE public.maya_messages (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Función SECURITY DEFINER para verificar ownership
...
-- Trigger para actualizar updated_at cuando se agrega un mensaje
...
-- Trigger para actualizar updated_at cuando se modifica una conversación
