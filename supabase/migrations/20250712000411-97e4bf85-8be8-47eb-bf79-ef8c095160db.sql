...
-- 1. Identificar y limpiar empleados duplicados por cédula y empresa
WITH duplicate_employees AS (
  SELECT 
    cedula,
    company_id,
    COUNT(*) as duplicate_count,
    array_agg(id ORDER BY created_at DESC) as employee_ids, -- Más reciente primero
...
-- 2. Preparar consolidación de datos (mantener el más reciente)
consolidation_plan AS (
  SELECT 
    cedula,
    company_id,
    employee_ids[1] as keep_id, -- El más reciente
    employee_ids[2:] as delete_ids -- Los demás para eliminar
  FROM duplicate_employees
)
-- 3. Actualizar referencias en tablas relacionadas hacia el empleado que se mantiene
UPDATE payrolls 
...
WITH duplicate_employees AS (
  SELECT 
    cedula,
    company_id,
    array_agg(id ORDER BY created_at DESC) as employee_ids
  FROM employees 
  WHERE cedula IS NOT NULL AND cedula != ''
  GROUP BY cedula, company_id 
  HAVING COUNT(*) > 1
...
consolidation_plan AS (
  SELECT 
    cedula,
    company_id,
    employee_ids[1] as keep_id,
    employee_ids[2:] as delete_ids
  FROM duplicate_employees
)
...
-- 7. Eliminar empleados duplicados (mantener solo el más reciente)
WITH duplicate_employees AS (
  SELECT 
    cedula,
    company_id,
...
consolidation_plan AS (
  SELECT 
    unnest(employee_ids[2:]) as delete_id
  FROM duplicate_employees
)
DELETE FROM employees 
WHERE id IN (SELECT delete_id FROM consolidation_plan);
