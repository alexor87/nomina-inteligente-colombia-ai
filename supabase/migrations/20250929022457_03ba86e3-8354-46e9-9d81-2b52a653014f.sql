...
-- Create the execute_safe_query function for MAYA
...
BEGIN
  ...
  INSERT INTO public.security_audit_log (
    ...
    LEFT(query_sql, 500), -- Store first 500 chars for audit
    jsonb_build_object('query_length', LENGTH(query_sql), 'timestamp', now()),
    auth.uid(), company_id_param
  );
  
  -- Execute the query and return results as jsonb
  EXECUTE 'SELECT jsonb_agg(row_to_json(t)) FROM (' || query_sql || ') t' INTO result_data;
