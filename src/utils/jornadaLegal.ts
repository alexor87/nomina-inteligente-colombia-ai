...
export const getJornadaLegal = getJornadaInfo;

/**
 * ✅ KISS: Calcular valor hora extra usando jornada dinámica
 */
export function calcularValorHoraExtra(salarioBase: number, fecha: Date): number {
  const divisorHorario = getHourlyDivisor(fecha);
  const valorHoraOrdinaria = salarioBase / divisorHorario;
  return valorHoraOrdinaria * 1.25; // 25% recargo hora extra
}

/**
 * ✅ KISS: Obtener próximo cambio de jornada
 */
export function getProximoCambioJornada(fecha: Date) {
  const fechaString = fecha.toISOString().split('T')[0];
  
  if (fechaString < FECHA_PRIMERA_FASE) {
    return {
      fechaVigencia: new Date(FECHA_PRIMERA_FASE),
      horasSemanales: 46,
      descripcion: "Primera fase de reducción"
    };
  }
  
  if (fechaString < FECHA_SEGUNDA_FASE) {
    return {
      fechaVigencia: new Date(FECHA_SEGUNDA_FASE),
      horasSemanales: 44,
      descripcion: "Segunda fase de reducción"
    };
  }
  
  if (fechaString < FECHA_TERCERA_FASE) {
    return {
      fechaVigencia: new Date(FECHA_TERCERA_FASE),
      horasSemanales: 42,
      descripcion: "Tercera fase de reducción"
    };
  }
  
  if (fechaString < FECHA_CUARTA_FASE) {
    return {
      fechaVigencia: new Date(FECHA_CUARTA_FASE),
      horasSemanales: 40,
      descripcion: "Cuarta fase de reducción (final)"
    };
  }
  
  return null; // Ya está en la fase final
}

/**
 * ✅ KISS: Generar tooltip informativo para jornada
 */
export function getJornadaTooltip(fecha: Date): string {
  const jornada = getJornadaInfo(fecha);
  const proximoCambio = getProximoCambioJornada(fecha);
  
  let tooltip = `Jornada actual: ${jornada.horasSemanales}h semanales (${jornada.horasMensuales}h mensuales).\n`;
  tooltip += `Ley aplicable: ${jornada.ley}.\n`;
  tooltip += `${jornada.descripcion}.`;
  
  if (proximoCambio) {
    tooltip += `\n\nPróximo cambio: ${proximoCambio.horasSemanales}h semanales desde el ${proximoCambio.fechaVigencia.toLocaleDateString('es-CO')}.`;
  }
