...
import { ConflictResolutionPanel } from '@/components/absence-integration/ConflictResolutionPanel';
import { useCurrentCompany } from '@/hooks/useCurrentCompany';

interface AbsenceConflictAlertProps {
  onConflictsResolved?: () => void;
}

export const AbsenceConflictAlert: React.FC<AbsenceConflictAlertProps> = ({
  onConflictsResolved
}) => {
  ...
  const checkForConflicts = async () => {
    ...
    try {
      // Verificar conflictos para el periodo actual
  ...
  if (isCheckingConflicts) {
    return (
      <Alert className="border-blue-200 bg-blue-50">
        ...
        <AlertDescription>
          Verificando consistencia entre ausencias y novedades...
        </AlertDescription>
      </Alert>
    );
  }
    ...
    return (
      <Alert className="border-green-200 bg-green-50">
        ...
        <AlertDescription className="text-green-800">
          No hay conflictos detectados entre ausencias y novedades
        </AlertDescription>
      </Alert>
    );
  }
      ...
      <Card className="border-orange-200 bg-orange-50">
        <CardHeader>
          <CardTitle className="text-orange-800">
            Resolucion de Conflictos de Ausencias
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <AlertDescription className="flex items-center justify-between">
        <div>
          ...
          <span className="text-orange-700 ml-2">
            {conflictHook.conflictReport?.totalConflicts} inconsistencias entre ausencias y novedades
          </span>
        </div>
        <Button
          variant="outline"
